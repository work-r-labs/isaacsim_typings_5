...
import typing
import warp as wp
__all__: list[str] = ['PhysicsMaterial', 'SurfaceDeformableMaterial', 'Usd', 'carb', 'deformableUtils', 'np', 'ops_utils', 'physx_bindings', 'prim_utils', 'stage_utils', 'wp']
class SurfaceDeformableMaterial(isaacsim.core.experimental.materials.impl.physics_materials.physics_material.PhysicsMaterial):
    """
    High level wrapper for creating/encapsulating Surface Deformable material prims.
        ...
        .. note::
            ...
            * If the prim paths do not exist, Surface Deformable material prims are created at each path
        ...
        .. code-block:: python
            ...
            >>> prims = SurfaceDeformableMaterial(paths)  # doctest: +NO_CHECK
    ...
    def get_poissons_ratios(self, *, indices: list | np.ndarray | wp.array | None = None) -> wp.array:
        ...
        """
    def get_surface_stiffnesses(self, *, indices: list | np.ndarray | wp.array | None = None) -> tuple[wp.array, wp.array, wp.array]:
        """
        Get the surface stretch, shear and bend stiffnesses of the prims.
                ...
                Returns:
                    Three-elements tuple. 1) The surface stretch stiffnesses (shape ``(N, 1)``).
                    2) The surface shear stiffnesses (shape ``(N, 1)``). 3) The surface bend stiffnesses (shape ``(N, 1)``).
        ...
        """
    def get_surface_thicknesses(self, *, indices: list | np.ndarray | wp.array | None = None) -> wp.array:
        """
    ...
    def set_poissons_ratios(self, poissons_ratios: list | np.ndarray | wp.array, *, indices: list | np.ndarray | wp.array | None = None) -> None:
        ...
        """
    def set_surface_stiffnesses(self, stretch_stiffnesses: list | np.ndarray | wp.array | None = None, shear_stiffnesses: list | np.ndarray | wp.array | None = None, bend_stiffnesses: list | np.ndarray | wp.array | None = None, *, indices: list | np.ndarray | wp.array | None = None) -> None:
        """
        Set the surface stretch, shear and bend stiffness of the prims.
                ...
                Args:
                    stretch_stiffnesses: Surface stretch stiffnesses (shape ``(N, 1)``).
                        If the input shape is smaller than expected, data will be broadcasted (following NumPy broadcast rules).
                    shear_stiffnesses: Surface shear stiffnesses (shape ``(N, 1)``).
                        If the input shape is smaller than expected, data will be broadcasted (following NumPy broadcast rules).
                    bend_stiffnesses: Surface bend stiffnesses (shape ``(N, 1)``).
                        If the input shape is smaller than expected, data will be broadcasted (following NumPy broadcast rules).
                    indices: Indices of prims to process (shape ``(N,)``). If not defined, all wrapped prims are processed.
        ...
        """
    def set_surface_thicknesses(self, surface_thicknesses: list | np.ndarray | wp.array, *, indices: list | np.ndarray | wp.array | None = None) -> None:
        """
        Set the surface thicknesses of the prims.
                ...
                Args:
                    surface_thicknesses: Surface thicknesses (shape ``(N, 1)``).
                        If the input shape is smaller than expected, data will be broadcasted (following NumPy broadcast rules).
                    indices: Indices of prims to process (shape ``(N,)``). If not defined, all wrapped prims are processed.
