...
import typing
import warp as wp
__all__: list[str] = ['CylinderLight', 'Light', 'Usd', 'UsdLux', 'np', 'ops_utils', 'stage_utils', 'wp']
class CylinderLight(isaacsim.core.experimental.objects.impl.lights.light.Light):
    """
    High level class for creating/wrapping USD Cylinder Light (light emitted outward from a cylinder) prims.
        ...
        Args:
            ...
            lengths: Lengths (cylinder's spine length along the axis) (shape ``(N, 1)``).
                If the input shape is smaller than expected, data will be broadcasted (following NumPy broadcast rules).
        ...
        .. code-block:: python
            ...
            >>> prims = CylinderLight(paths)  # doctest: +NO_CHECK
    ...
    def are_of_type(paths: str | Usd.Prim | list[str | Usd.Prim]) -> wp.array:
        ...
        """
    def __init__(self, paths: str | list[str], *, radii: list | np.ndarray | wp.array | None = None, lengths: list | np.ndarray | wp.array | None = None, positions: list | np.ndarray | wp.array | None = None, translations: list | np.ndarray | wp.array | None = None, orientations: list | np.ndarray | wp.array | None = None, scales: list | np.ndarray | wp.array | None = None, reset_xform_op_properties: bool = False) -> None:
        ...
    ...
    def get_radii(self, *, indices: list | np.ndarray | wp.array | None = None) -> wp.array:
        ...
        """
    def set_enabled_treat_as_lines(self, enabled: list | np.ndarray | wp.array, *, indices: list | np.ndarray | wp.array | None = None) -> None:
        """
        Enable or disable the treat as lines (effectively, a zero-radius cylinder) of the prims.
                ...
                Args:
                    enabled: Boolean flags to enable/disable treat as lines (shape ``(N, 1)``).
                        If the input shape is smaller than expected, data will be broadcasted (following NumPy broadcast rules).
                    indices: Indices of prims to process (shape ``(N,)``). If not defined, all wrapped prims are processed.
        ...
        """
    def set_lengths(self, lengths: list | np.ndarray | wp.array, *, indices: list | np.ndarray | wp.array | None = None) -> None:
        """
